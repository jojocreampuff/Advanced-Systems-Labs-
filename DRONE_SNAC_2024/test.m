...
for i = 1:max_training_loop
    ...
    KEEP_DIMS = zeros(N_states, N_patterns);
    for j = 1 : N_patterns
        % Target costate equation
        A_k_plus_1 = A(x_k_plus_1(:,j));
        KEEP_DIMS(:,j) = A_k_plus_1.' * lambda_k_plus_2(:,j);
    end
 % keep_dims must be 6x # train
    lambda_k_plus_1_target = Attitude_Q * (x_k_plus_1) + KEEP_DIMS;
    Last_W = Attitude_W;
