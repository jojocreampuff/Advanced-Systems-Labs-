...
x_train_init = gpuArray(x_train_init);
target_init = gpuArray(target_init);
Attitude_Q = gpuArray(Attitude_Q);
Attitude_R = gpuArray(Attitude_R);

tic
fprintf("starting training\n")

% Initialize and train the network on GPU
hiddenLayerSize = [10, 10, 10];
net = fitnet(hiddenLayerSize, 'traingdx');
net = configure(net, x_train_init, target_init); % Ensure net is configured before GPU transfer
net = train(net,x_train_init,target_init,'useParallel','yes','useGPU','yes');
y = net(x_train_init);
...
for i = 1:max_training_loop
    ...
    X1 = gpuArray(PHI_min + (PHI_max - PHI_min) * rand(1, N_patterns));
    X2 = gpuArray(THE_min + (THE_max - THE_min) * rand(1, N_patterns));
    X3 = gpuArray(PSI_min + (PSI_max - PSI_min) * rand(1, N_patterns));    
    X4 = gpuArray(p_min + (p_max - p_min) * rand(1, N_patterns));
    X5 = gpuArray(q_min + (q_max - q_min) * rand(1, N_patterns));
    X6 = gpuArray(r_min + (r_max - r_min) * rand(1, N_patterns)); 
    ...
    %% test/train error
    [mae_train(i), mse_train(i), mae_test(i), mse_test(i)] = z_deep_network_test_train(net, x_k, Attitude_G, Attitude_R, Attitude_F, Attitude_Q, discount, ...
...
% Ensure to transfer the final network back to the CPU if needed
mae_train = gather(mae_train);
mae_test = gather(mae_test);
mse_train = gather(mse_train);
mse_test = gather(mse_test);
net = gather(net);

figure(1);
iterations = 1:1:length((mae_test));
% Plot MAE
subplot(2, 1, 1);  % Create the first subplot in a 2x1 grid
plot(iterations, mae_train, '-o', 'DisplayName', 'MAE Train');
hold on;
plot(iterations, mae_test, '-o', 'DisplayName', 'MAE Test');
title('MAE vs. Iterations');
xlabel('Iterations');
ylabel('MAE');
legend('show');
grid on;
...
plot(iterations, mse_train, '-o', 'DisplayName', 'MSE Train');
hold on;
plot(iterations, mse_test, '-o', 'DisplayName', 'MSE Test');
