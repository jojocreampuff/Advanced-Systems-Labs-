function AC_results = simulate_AC_4drone(seed,comm, position, attitude, global_parameters, ref, IC_all, sim_params, Wxyz)
rng(seed);
...
%% different communication pathways
    ...
    if mod(i-1, 10) == 0
        ...
        U_pos(:,i,1) = Vk_pos' * a_Offline_AC_phi(pos_error(:,i,1)) - [0;0;g];
        U_pos(:,i,2) = Vk_pos' * a_Offline_AC_phi(pos_error(:,i,2)) - [0;0;g];
        U_pos(:,i,3) = Vk_pos' * a_Offline_AC_phi(pos_error(:,i,3)) - [0;0;g];
        U_pos(:,i,4) = Vk_pos' * a_Offline_AC_phi(pos_error(:,i,4)) - [0;0;g];
...
%% solve once every 10 timesteps (rates are held but look messy)
    ...
    torques(:,i,1) = Vk_att'*a_Offline_AC_phi(att_error(:,i,1));
    torques(:,i,2) = Vk_att'*a_Offline_AC_phi(att_error(:,i,2));
    torques(:,i,3) = Vk_att'*a_Offline_AC_phi(att_error(:,i,3));
    torques(:,i,4) = Vk_att'*a_Offline_AC_phi(att_error(:,i,4));
...
AC_results.X = X;
AC_results.U = U;
AC_results.X_ref = X_ref;
AC_results.X1 = X(1:12,:,1);
AC_results.X2 = X(1:12,:,2);
AC_results.X3 = X(1:12,:,3);
AC_results.X4 = X(1:12,:,4);
AC_results.U1 = U(:,:,1);
AC_results.U2 = U(:,:,2);
AC_results.U3 = U(:,:,3);
AC_results.U4 = U(:,:,4);
AC_results.X1_ref = X_ref(:,:,1);
AC_results.X2_ref = X_ref(:,:,2);
AC_results.X3_ref = X_ref(:,:,3);
AC_results.X4_ref = X_ref(:,:,4);

AC_results.error.pos = pos_error;
AC_results.error.att = att_error;
AC_results.error.all = cat(1, pos_error, att_error);

AC_results.U_pos = U_pos;

AC_results.simtime = AC_sim_time;
AC_results.tracking_error = tracking_error_xyz';
AC_results.tracking_error_norm = tracking_error_xyz_norm;

AC_results.formation_error = formation_error;
AC_results.formation_error_norm = formation_error_norm;
AC_results.formation_error_int = formation_error_int;

AC_results.r_smooth1 = squeeze(X_ref(1:6,:,1));
AC_results.r_smooth2 = squeeze(X_ref(1:6,:,2));
AC_results.r_smooth3 = squeeze(X_ref(1:6,:,3));
AC_results.r_smooth4 = squeeze(X_ref(1:6,:,4));

AC_results.time = T;
AC_results.offsets = offsets;
end