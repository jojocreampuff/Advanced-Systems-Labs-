function Flex_SNAC_results = simulate_Flex_SNAC_4drone_ANALYSIS(seed,comm, position, attitude, global_parameters, ref, IC_all, sim_params)
rng(seed);
...
K = length(Attitude_Wflex(1,1,:));
fprintf("Does the horizon change? K = %f\n",K)
...
Wxyz = gen_wind_vector(global_parameters.W_nominal,dt,tf, seed);
...
%% different communication pathways
    ...
    %% tracking metrics, remove if not needed
    ...
    %% formation metrics, remove if not needed
