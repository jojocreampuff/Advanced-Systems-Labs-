function lqr_results = simulate_LQR_4drone(seed,comm, position, attitude, global_parameters, ref, IC_all, sim_params, Wxyz)
rng(seed);
...
N1 = floor(N/3);
N2 = floor(2*N/3);
% Followers maintain offsets relative to leader
offsets = zeros(3, Nd);
offsets(:,1) = [ 0;  0; 0];   % leader
offsets(:,2) = [-1;  0; 0];
offsets(:,3) = [-1; -1; 0];
offsets(:,4) = [-1;  1; 0];
k_spring = 0;  % Tunable spring foce constant/gain
k_damp = 0;

%% LQR
[K_pos, K_att] = get_LQR_gains(10*Q_pos/dt,10*R_pos/dt, [300,300,100,10,10,10].*Q_att/dt, 1*R_att/dt, Ix, Iy, Iz);
...
%% different communication pathways
    ...
    spring_pos_error2=(X(1:3,i,2) - (X(1:3,i,1) + offsets(:,2)));
    spring_pos_error3=(X(1:3,i,3) - (X(1:3,i,1) + offsets(:,3)));
    spring_pos_error4=(X(1:3,i,4) - (X(1:3,i,1) + offsets(:,4)));

    damp_vel_error2= X(4:6,i,2) - X(4:6,i,1);
    damp_vel_error3= X(4:6,i,3) - X(4:6,i,1);
    damp_vel_error4= X(4:6,i,4) - X(4:6,i,1);
    ...
    if mod(i-1, 10) == 0
        U_pos(:,i,1) = -K_pos * pos_error(:,i,1) + [0;0;g];
        U_pos(:,i,2) = -K_pos * pos_error(:,i,2) + [0;0;g];
        U_pos(:,i,3) = -K_pos * pos_error(:,i,3) + [0;0;g];
        U_pos(:,i,4) = -K_pos * pos_error(:,i,4) + [0;0;g];
    
        U_pos(:,i,2) = U_pos(:,i,2) - k_spring*spring_pos_error2 - k_damp*damp_vel_error2;
        U_pos(:,i,3) = U_pos(:,i,3) - k_spring*spring_pos_error3 - k_damp*damp_vel_error3;
        U_pos(:,i,4) = U_pos(:,i,4) - k_spring*spring_pos_error4 - k_damp*damp_vel_error4;
    ...
    torques(:,i,1) = -K_att * att_error(:,i,1);
    torques(:,i,2) = -K_att * att_error(:,i,2);
    torques(:,i,3) = -K_att * att_error(:,i,3);
    torques(:,i,4) = -K_att * att_error(:,i,4);
...
lqr_results.X1 = X(1:12,:,1);
lqr_results.X2 = X(1:12,:,2);
lqr_results.X3 = X(1:12,:,3);
lqr_results.X4 = X(1:12,:,4);
lqr_results.U1 = U(:,:,1);
lqr_results.U2 = U(:,:,2);
lqr_results.U3 = U(:,:,3);
lqr_results.U4 = U(:,:,4);
lqr_results.X1_ref = X_ref(:,:,1);
lqr_results.X2_ref = X_ref(:,:,2);
lqr_results.X3_ref = X_ref(:,:,3);
lqr_results.X4_ref = X_ref(:,:,4);

lqr_results.error.pos = pos_error;
lqr_results.error.att = att_error;
lqr_results.error.all = cat(1, pos_error, att_error);

lqr_results.U_pos = U_pos;

% lqr_results.inscost.pos = instant_cost_pos;
% lqr_results.inscost.att = instant_cost_att;
% lqr_results.cumcost.pos = cumulative_cost_pos;
% lqr_results.cumcost.att = cumulative_cost_att;
% lqr_results.cumcost.total = cumulative_cost_pos + cumulative_cost_att;

lqr_results.simtime = lqr_sim_time;
lqr_results.gains.K_pos = K_pos;
lqr_results.gains.K_att = K_att;

lqr_results.tracking_error = tracking_error_xyz';
lqr_results.tracking_error_norm = tracking_error_xyz_norm;

lqr_results.formation_error = formation_error;
lqr_results.formation_error_norm = formation_error_norm';
lqr_results.formation_error_int = formation_error_int;

% lqr_results.all_cost = all_cost;
% lqr_results.team_cost = [team_total_cost, team_tracking_cost, team_control_cost];

lqr_results.r_smooth1 = squeeze(X_ref(1:6,:,1));
lqr_results.r_smooth2 = squeeze(X_ref(1:6,:,2));
lqr_results.r_smooth3 = squeeze(X_ref(1:6,:,3));
lqr_results.r_smooth4 = squeeze(X_ref(1:6,:,4));

lqr_results.time = T;
lqr_results.offsets = offsets;

end



% function lqr_results = simulate_LQR_4drone(seed, position, attitude, global_parameters, ref, IC_all, sim_params)
% rng(seed);
% R_pos = position.R_pos;
% Q_pos = position.Q_pos;
% R_att = attitude.R_att;
...
%         end
%         if wind_logic == 1
%             [axyz_uncert, alphaxyz_uncert] = wind_f_m(Wxyz(:,i), X(:,i,d), rho, Cd, A, r, m, Ix, Iy, Iz);
%             X(:,i,d) = X(:,i,d) + dt * [0;0;0;axyz_uncert;0;0;0;alphaxyz_uncert];
%         end
%     end
% 
...
% % if i <= N1
% %     %interval a
% %     % 1 communicates with 2 and 4, 3 communicates with 2 and 4
% %     X_ref(1:6,i,1) = r_smooth_leader(:,i); % leader drone reference
% %     X_ref(1:6,i,2) = [X(1:3,i,1) + offsets(:,2); X(4:6,i,1)];
% %     X_ref(1:6,i,3) = [(X(1:3,i,2)+X(1:3,i,4))/2 + offsets(:,3); (X(4:6,i,2)+X(4:6,i,4))/2];
% %     X_ref(1:6,i,4) = [X(1:3,i,1) + offsets(:,4); X(4:6,i,1)];
% % elseif i <= N2
% %     %interval b
% %     % 1 communicates with 2, 2 with 3, 3 with 4
% %     X_ref(1:6,i,1) = r_smooth_leader(:,i); % leader drone reference
% %     X_ref(1:6,i,2) = [X(1:3,i,1) + offsets(:,2); X(4:6,i,1)];
% %     X_ref(1:6,i,3) = [X(1:3,i,2) + offsets(:,3); X(4:6,i,2)];
% %     X_ref(1:6,i,4) = [X(1:3,i,3) + offsets(:,4); X(4:6,i,3)];
% % else
% %     %interval c (same as a)
% %     X_ref(1:6,i,1) = r_smooth_leader(:,i); % leader drone reference
% %     X_ref(1:6,i,2) = [X(1:3,i,1) + offsets(:,2); X(4:6,i,1)];
% %     X_ref(1:6,i,3) = [(X(1:3,i,2)+X(1:3,i,4))/2 + offsets(:,3); (X(4:6,i,2)+X(4:6,i,4))/2];
% %     X_ref(1:6,i,4) = [X(1:3,i,1) + offsets(:,4); X(4:6,i,1)];
