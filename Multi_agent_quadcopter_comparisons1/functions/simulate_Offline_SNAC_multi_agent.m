function Offline_SNAC_results = simulate_Offline_SNAC_multi_agent(seed,position, attitude, global_parameters, ref, IC, sim_params)
rng(seed);
...
for i = 1:length(T)
    ...
    if mod(i-1,10) == 0
        ...
        U_pos(:,i) =  -Position_R^-1 * Position_G(pos_error(:,i))' * Position_W' * a_Offline_SNAC_phi_pos(pos_error_future);
