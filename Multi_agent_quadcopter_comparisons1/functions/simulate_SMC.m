function SMC_results = simulate_SMC(seed,position, attitude, global_parameters, ref, IC, sim_params, Wxyz)
...
for i = 1:length(T)
    ...
    %% SMC Position (NO HOLD) DONT DELETE
    ...
    %     w0 = 15;  % try 5–15
    %     k_pos =.5*[1;1; 1];
    %     [ieso_state, d_hat_force, e_obs] = IESO_position_update(ieso_state, X(:,i), U_pos(:,i), m, 10*dt, w0);
    ...
    if mod(i-1,10) == 0
        %% I removed the look ahead
        % idx = min(i + 10, N);
        r_future = r_smooth(:, i);
        r_xzy_ddot_future = r_xyz_ddot(:,i);
        pos_err_future = X(1:6, i) - r_future;
        S_pos = C_pos*pos_err_future(1:3) + pos_err_future(4:6); % C_pos*e + e_dot
        S_pos = max(-1, min(1, S_pos ./ eps_pos)); % bounrdy 
        U_pos(:,i) = r_xzy_ddot_future - C_pos*pos_err_future(4:6) - K_xyz*S_pos; % desired accelerations in x y and z
            %% position Improved ESO from paper 
        if use_ESO == 1 && wind_logic == 1
            w0 = 1;  % try 5–15
            k_pos =.1*[1;1; 1];
