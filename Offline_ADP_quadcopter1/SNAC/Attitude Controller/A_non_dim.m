% verify that this function is good
function A_bar = A_non_dim(x_bar, dt, Ix, Iy, Iz, x1_max , x2_max, x3_max, x4_max, x5_max, x6_max)

% row 1 checked
    A_11 = (dt*(x_bar(5)*x1_max*x4_max*cos(x_bar(1)*x1_max)*tan(x_bar(2)*x2_max) - x_bar(6)*x1_max*x6_max*sin(x_bar(1)*x1_max)*tan(x_bar(2)*x2_max)))/x1_max + 1;
    A_12 = (dt*(x_bar(5)*x2_max*x4_max*sin(x_bar(1)*x1_max)*(tan(x_bar(2)*x2_max)^2 + 1) + x_bar(6)*x2_max*x6_max*cos(x_bar(1)*x1_max)*(tan(x_bar(2)*x2_max)^2 + 1)))/x1_max;
    A_13 = 0;
    A_14 = (dt*x4_max)/x1_max;
    A_15 = (dt*x4_max*sin(x_bar(1)*x1_max)*tan(x_bar(2)*x2_max))/x1_max;
    A_16 = (dt*x6_max*cos(x_bar(1)*x1_max)*tan(x_bar(2)*x2_max))/x1_max;
% row 2 checked
