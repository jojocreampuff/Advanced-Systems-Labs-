...
% import training set
    ...
    lambda_k_2_train = net(x_k_1_train);
    ...
    lambda_k_1_target_train = Attitude_Q * (x_k_1_train) + discount*KEEP_DIMS;

    error(:, :) = lambda_k_1_target_train - lambda_k_1_train;
    mae_train = mae(error(:,:));
    mse_train = mean(mean((error(:,:).^2) ));
    ...
    lambda_k_1_target_test = Attitude_Q * (x_k_1_test) + discount*KEEP_DIMS;

    error(:, :) = lambda_k_1_target_test - lambda_k_1_test;
    mae_test = mae(error(:,:));
    mse_test = mean(mean((error(:,:).^2) ));
