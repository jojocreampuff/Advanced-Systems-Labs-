...
load('SNAC_pos_NEW.mat','Position_W','Position_Q','Position_R','Position_F','Position_G','dt')
load('SNAC_att_NEW.mat','Attitude_W','Attitude_Q','Attitude_R','Attitude_F','Attitude_G','Attitude_G_bar','max_states')
dt = 0.004;
...
for seed = 1:length(random_seeds)
    ...
    for i = 1:length(T)
        ...
        torques(:,i) = -Attitude_R^-1 * Attitude_G_bar(Att_error(:,i)./max_states)' * Attitude_W' * Basis_Func_84(Att_error(:,i)./max_states);
    ...
    SNAC_final_cost = SNAC_c_cost_total(end-1);
    fieldname = sprintf("SNAC_results%d", seed); % Or use seed index
    SNAC_runs_case3.(fieldname) = SNAC_results;
    SNAC_costs_MC(seed,1) = SNAC_final_cost;
    SNAC_error_MC(seed,1) = SNAC_tracking_error_sum;
end

save("SNAC_MC_case3.mat","SNAC_runs_case3","SNAC_error_MC","SNAC_costs_MC")
