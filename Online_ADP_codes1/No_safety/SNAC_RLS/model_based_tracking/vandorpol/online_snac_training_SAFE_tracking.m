function out = online_snac_training_SAFE_tracking(t,initials)
...
%% extract inputs
    ...
    %% reference and error and barrier info
    ...
    %% Noisey signal (gated based on h(x))
    u_prob = u_excite(t,amp_mult);
    delta_h = 0.5;  % gate: 0 at/near boundary, 1 when comfortably inside
    gate = min( max(h / delta_h, 0) , 1);
    u_exc = gate*u_prob(1);

    %% exploration step and update
    if t<=(tf*.9)
        unew = u + u_exc;
