function sim = simulate_closed_loop(W, params)
% ub_logic added or removes ub at runtime 
    ...
    odefun = @(t,x) f(x) + g*compute_u(t, x, W, g, params);
    tspan  = [0, params.T_sim];
    x0     = .2*randn(params.nx,1);

    ode_opts_sim = odeset('RelTol',1e-3,'AbsTol',1e-6,'MaxStep',params.dt);
    ...
    for i = 1:N
        ...
        u_out(:,i) = compute_u(time(i), x_out(:,i), W, g, params);
...
function u = compute_u(t, x, W, g, params)
    [r, ~] = ref_refdot(t);
    e   = x - r;
    phi = PHI(e, r);
