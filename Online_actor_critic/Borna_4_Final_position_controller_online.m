...
alpha_1 = 100;
alpha_2 = 50;
F1 = 20 * eye(no_neurns_critic); 
F2 = 11 * eye(no_neurns_critic); 
...
for j = 1:epoch
    ...
    if mod(j,10) == 0
        x(:,1) = zeros(N_states,1) + [(j-1)*randn(N_states/2,1); (j-1)/(j+1)*randn(N_states/2,1)];
    end
    ...
    for i = 1:N-1
        ...
        excitation_wave_1(i) = 1*exp(-0.001*t)* sin(2*t);
