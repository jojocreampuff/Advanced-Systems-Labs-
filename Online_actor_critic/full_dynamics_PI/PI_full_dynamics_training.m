...
for i = 1:N-1
    ...
    x_val = x()
    GRAD_PHI = subs(GRAD_PHI_sym, {x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12}, num2cell(x_val));

    u2(:,i)  = -0.5 * R^-1 * Full_g_bar(x(:,i)).' * GRAD_PHI.' * W2(:,i); % (39

    f_plus_gu = Full_f_bar(x(:,i)) + Full_g_bar(x(:,i))* u2(:,i); 
