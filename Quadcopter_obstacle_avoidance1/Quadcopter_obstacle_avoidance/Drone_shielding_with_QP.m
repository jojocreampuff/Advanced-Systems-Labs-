function [U_pos, h_raw, delta] = Drone_shielding_with_QP(u_nom, x_pos, pos_error,W_pos,t,useSafety)
...
(sin(t/5)*((981*exp(-t/100))/100 - 981/100))/25 + (981*cos(t/5)*exp(-t/100))/25000 - (981*sin(t/5)*exp(-t/100))/1000000
    ...
    % safety terms 
    gamma_cbf = 1*.05;     % CBF gamma in  gamma/Bbar
    k0 = 1;                   % stiffness
    ...
    if h < 50 && useSafety == 1
        ...
        gradV = a_Discounted_Single_Critic_Grad_phi_pos(e_i)'*W_pos;    % approximated dV/de needed for CLF
        ...
        % In standard quadprog form: 0.5*U'H\U + f'U
