function Discounted_Single_Critic_results = simulate_Discounted_Single_Critic(position, attitude, global_parameters, ref, IC, sim_params,useSafety)
...
for i = 1:length(T)
    ...
    u_nom = -0.5*R_pos_DSC^-1*g_pos'*a_Discounted_Single_Critic_Grad_phi_pos(pos_error(:,i))'*W_pos + [0; 0; g];
    
    [U_pos(:,i), h_log(i), delta(i)] = Drone_shielding_with_QP(u_nom, X(1:6, i), pos_error(:,i),W_pos,T(i),useSafety);
...
sim_time = toc
% avrg_qp_loop_time = mean(loop_with_qp)
% avrg_no_qp_loop_time = mean(time_no_qp)
...
Discounted_Single_Critic_results.h_log  = h_log;
Discounted_Single_Critic_results.delta = delta;
...
Discounted_Single_Critic_results.U_pos = U_pos; 
Discounted_Single_Critic_results.simtime =sim_time;
