function Offline_AC_results = simulate_Offline_AC(position, attitude, global_parameters, ref, IC, sim_params)
...
%% offline AC stuff
load("z_offline_ac_pos_data.mat","Vk")
Vk_pos = Vk;
load("z_offline_ac_att_data.mat", "Vk")
Vk_att = Vk;
...
for i = 1:length(T)
    ...
    U_pos(:,i) =  Vk_pos' * a_Offline_AC_phi(pos_error(:, i)) - [0;0;g];
    % U_pos(3,i) = U_pos(3,i) - 9.81;
...
Offline_AC_results.inscost = [instant_cost_pos; instant_cost_att];
Offline_AC_results.cumcost = [cumulative_cost_pos; cumulative_cost_att];
Offline_AC_c_cost_total = sum(Offline_AC_results.cumcost, 1);
Offline_AC_results.Offline_AC_c_cost_total = Offline_AC_c_cost_total;
